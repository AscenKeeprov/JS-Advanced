class Warehouse {
	...
	set capacity(givenSpace) {
		...
		} else throw 'Invalid given warehouse space';
	}
	addProduct(type, product, quantity) {
		...
		} else throw 'There is not enough space or the warehouse is already full';
		return output;
	}
	occupiedCapacity() {
		let output = 0;
	...
	orderProducts(type) {
		...
		for (let product of sortedKeys) {
			if (newObj.hasOwnProperty(product) === false) newObj[product] = 0;
	...
	revision() {
		...
		} else output = 'The warehouse is empty';
		return output.trim();
	}
	scrapeAProduct(product, quantity) {
		...
		if (type !== undefined) {
			if (quantity <= this.availableProducts[type][product]) {
				this.availableProducts[type][product] -= quantity;
			} else this.availableProducts[type][product] = 0;
			output = this.availableProducts[type];
		} else throw `${product} do not exists`;
		return output;
	}
}
